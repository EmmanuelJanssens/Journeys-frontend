...
class JourneysMap {
    ...
    createMarker(imageUrl: string, lng: number, lat: number, bgSize: string, size: string) {
        ...
        return new mapboxgl.Marker(marker).setLngLat(new mapboxgl.LngLat(lng, lat)).addTo(this.map!);
    }

    addJourneysExperiencesLayer(data: GeoJSON.FeatureCollection): mapboxgl.Map | undefined {
        this.clearMap();
        if (this.map) {
            const source = this.map.getSource(this.MapLayer.journey_experiences) as mapboxgl.GeoJSONSource;
            ...
            this.existingLayers.push(this.MapLayer.journey_experiences + "_route_label");
    ...
    addJourneyListLayer(data: GeoJSON.FeatureCollection): mapboxgl.Map | undefined {
        this.clearMap();
        if (this.map) {
            const source = this.map.getSource(this.MapLayer.journey_list) as mapboxgl.GeoJSONSource;
    ...
    addPoiListLayer(data: GeoJSON.FeatureCollection): mapboxgl.Map | undefined {
        ...
        if (this.map) {
            this.map.off("click", () => {});
            const source = this.map.getSource(this.MapLayer.poi_list) as mapboxgl.GeoJSONSource;
            if (source) {
                source.setData(data);
            } else {
            ...
            const lines = data.features.filter((feature) => feature.geometry.type == "LineString");
            const route_source = this.map.getSource(this.MapLayer.poi_list + "_route") as mapboxgl.GeoJSONSource;
            if (route_source) {
    ...
    addStopPoint(data: GeoJSON.Feature) {
        if (this.map) {
            const route = this.map.getSource(this.MapLayer.poi_list + "_route") as mapboxgl.GeoJSONSource;
            if (route) {
                route.setData(data);
            }
        }
...
export const mapInstance = new JourneysMap();
export const mapLayers = mapInstance.MapLayer;
